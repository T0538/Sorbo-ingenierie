      ...
      <article class="software-card" aria-label="${nom}" data-logiciel-id="${logiciel.id}" style="background: #ffffff; border-radius: 12px; box-shadow: 0 6px 25px rgba(20,20,40,0.08); overflow: hidden; display: grid; grid-template-columns: 1fr; max-width: 400px; margin: 0 0 20px 0; width: 100%;">
        ...
        <div class="software-content" style="padding: 20px 24px; text-align: left; position: relative; z-index: 2;">
          ...
          <div class="software-buttons" style="display:flex; flex-direction:column; gap:8px; position: relative; z-index: 3;">
            ...
            <a href="logiciel-details.html?id=${encodeURIComponent(logiciel.id || logiciel._id || '')}&nom=${encodeURIComponent(logiciel.nom || '')}" class="btn btn-secondary" style="padding:8px 14px; border-radius:6px; text-decoration:none; display:inline-flex; align-items:center; justify-content:center; gap:6px; font-weight:600; cursor:pointer; background:white; color:#374155; border:1px solid #d1d5db; font-size:0.9rem; pointer-events:auto; text-align:center;"><i class="fas fa-info-circle"></i> Infos</a>
          </div>
        </div>
      </article>
    `;
