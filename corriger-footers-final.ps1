# Script final pour corriger les footers des pages HTML
# Remplace complètement les footers problématiques par le footer standardisé
...
Write-Host "Correction finale des footers problematiques..." -ForegroundColor Yellow
...
foreach ($page in $pages) {
    if (Test-Path $page) {
        ...
        try {
            ...
            # Remplacer le footer complet avec une regex plus robuste
            $pattern = '(?s)\s*<footer>.*?</footer>\s*'

            if ($content -match $pattern) {
                $content = $content -replace $pattern, $footerStandard

                # Sauvegarder avec l'encodage UTF8
                $content | Out-File $page -Encoding UTF8
                Write-Host "Footer corrige pour $page" -ForegroundColor Green
            } else {
                Write-Host "Aucun footer trouve dans $page" -ForegroundColor Yellow
            }
        }
        catch {
        ...
        Write-Host "Page non trouvee : $page" -ForegroundColor Red
    }
}
